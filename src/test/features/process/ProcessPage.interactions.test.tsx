...
vi.mock("@/features/process/components/ElectionTypeToggle", () => ({
  ElectionTypeToggle: ({ onChange }: { onChange: (v: "lok_sabha" | "vidhan_sabha") => void }) => (
    <button onClick={() => { onChange("vidhan_sabha"); }}>change election</button>
  ),
}));
vi.mock("@/features/process/components/PhaseTabBar", () => ({
  PhaseTabBar: ({ onPhaseChange }: { onPhaseChange: (p: "pre-election" | "during-election" | "post-election") => void }) => (
    <button onClick={() => { onPhaseChange("during-election"); }}>change phase</button>
  ),
}));
...
describe("ProcessPage interactions", () => {
  it("updates params for steps hook and toggles card open/close", () => {
    ...
    render(<ProcessPage />);
    expect(useProcessStepsMock).toHaveBeenLastCalledWith("lok_sabha", "pre-election");

    fireEvent.click(screen.getByRole("button", { name: /change election/i }));
    expect(useProcessStepsMock).toHaveBeenLastCalledWith("vidhan_sabha", "pre-election");

    fireEvent.click(screen.getByRole("button", { name: /change phase/i }));
    expect(useProcessStepsMock).toHaveBeenLastCalledWith("vidhan_sabha", "during-election");
    ...
    expect(screen.getByRole("button", { name: "s1:closed" })).toBeInTheDocument();
  });
  ...
  it("opens and closes AI drawer", () => {
    ...
    expect(screen.queryByRole("button", { name: /close ai/i })).not.toBeInTheDocument();
  });
});
