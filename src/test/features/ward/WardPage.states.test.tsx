...
vi.mock("@/shared/hooks/useProfile", () => ({
  useProfile: () => ({ profile: { constituency: "X" } }),
}));
vi.mock("@/features/ward/hooks/usePartyData", () => ({
  usePartyData: () => ({ parties: [], isLoading: false, error: "party failed" }),
}));
...
describe("WardPage error branch", () => {
  it("shows aggregated top-level error state", () => {
    render(<WardPage />);
    expect(screen.getAllByText("party failed").length).toBeGreaterThan(0);
  });
});
