...
export const MyTicketsPanel = memo(function MyTicketsPanel() {
  ...
  const { user } = useAuth();
  const { tickets, isLoading, error, filter, setFilter } = useTickets(user?.uid);
  ...
  return (
    <div style={{ padding: "var(--space-md)", display: "flex", flexDirection: "column", gap: "var(--space-md)", height: "100%" }}>
      {/* Action Header */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h3 style={{ margin: 0, fontSize: "16px" }}>{t("support.tickets.title", "Track Issues")}</h3>
        <button 
