...
describe("MediaUpload branches", () => {
  it("shows invalid file type error for unsupported type", () => {
    ...
    expect(screen.getByText(/only images and videos are allowed/i)).toBeInTheDocument();
  });
  ...
  it("shows upload failed on storage error", async () => {
    ...
    fireEvent.change(input, { target: { files: [file] } });
    await waitFor(() => { expect(screen.getByText(/failed to upload media/i)).toBeInTheDocument(); });
  });
});
