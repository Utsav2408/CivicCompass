...
export const TicketCard = memo(function TicketCard({ ticket }: TicketCardProps) {
  const getStatusStyles = (status: string) => {
    switch (status) {
      ...
      case "resolved":
        return { bg: "var(--gd-l)", color: "var(--gd-dark)", label: "Resolved" };
      case "closed":
        return { bg: "var(--border)", color: "var(--text-muted)", label: "Closed" };
      default:
  ...
  return (
    ...
    >
      ...
      <div style={{ 
        ...
        {ticket.mediaUrl && <span style={{ fontSize: "11px", color: "var(--in)" }}>📎 Media attached</span>}
      </div>
    </div>
  );
});
