...
const geminiFixtures: Record<string, string> = {
  election_process: 'Nomination filing begins after the announcement of the election schedule by the Election Commission of India.',
  candidate_summary: 'Assets declared as per affidavit filed with Election Commission.',
  unknown_query: 'I cannot find this in trusted ECI sources. Please verify at eci.gov.in.',
}

function matchGeminiFixture(prompt: string): string {
  if (prompt.includes('election process')) return geminiFixtures['election_process'] ?? ''
  if (prompt.includes('candidate')) return geminiFixtures['candidate_summary'] ?? ''
